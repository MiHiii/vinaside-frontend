...
export const removeService = createAsyncThunk('service/removeService', async (id: string, { rejectWithValue }) => {
  try {
...
export const restoreService = createAsyncThunk('service/restoreService', async (id: string, { rejectWithValue }) => {
  try {
...
export const toggleServiceQuantity = createAsyncThunk(
...
export const fetchServiceUsage = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      const error = err as { response?: { data?: { message?: string } } };
      return rejectWithValue(error.response?.data?.message || 'Lỗi lấy thông tin service');
    }
  },
);
...
export const fetchServiceBookings = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      const error = err as { response?: { data?: { message?: string } } };
      return rejectWithValue(error.response?.data?.message || 'Lỗi lấy danh sách booking');
    }
  },
);
...
export const fetchServiceDetailedStats = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      const error = err as { response?: { data?: { message?: string } } };
      return rejectWithValue(error.response?.data?.message || 'Lỗi lấy thống kê chi tiết dịch vụ');
    }
  },
);
...
const serviceSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchServiceUsage.fulfilled, (state, action: PayloadAction<ServiceUsageData>) => {
        state.loading = false;
      ...
      .addCase(fetchServiceBookings.fulfilled, (state, action: PayloadAction<ServiceBookingData[]>) => {
        state.loading = false;
      ...
      .addCase(fetchServiceDetailedStats.fulfilled, (state, action: PayloadAction<ServiceDetailedStats>) => {
        state.loading = false;
...
export const { clearServiceError, clearServiceDetail, clearServiceUsage } = serviceSlice.actions;
