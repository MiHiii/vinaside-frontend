...
interface ServiceState {
  ...
  loading: boolean;
  error: string | null;
  // Pagination for serviceBookings
  serviceBookingsTotal?: number;
  serviceBookingsPage?: number;
  serviceBookingsLimit?: number;
  serviceBookingsTotalPages?: number;
}
...
export const toggleServiceQuantity = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      const error = err as { response?: { data?: { message?: string } } };
      return rejectWithValue(
        error.response?.data?.message || "Lỗi đổi trạng thái cho phép số lượng"
      );
    }
  }
);
...
export const fetchServiceBookings = createAsyncThunk(
  ...
  async (
    ...
    try {
      console.log(
        "fetchServiceBookings - Calling API with ID:",
        id,
        page,
        limit
      );
      const { data } = await serviceApi.getBookings(id, { page, limit });
      console.log("fetchServiceBookings - API response:", data);
      // API response có cấu trúc: { success: true, data: { data: [...], total, page, limit, totalPages } }
...
const serviceSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        toggleServiceQuantity.fulfilled,
        (state, action: PayloadAction<Service>) => {
          state.loading = false;
          state.services = state.services.map((s) =>
            s._id === action.payload._id ? action.payload : s
      ...
      .addCase(
        ...
        (
          ...
          state.serviceBookingsTotal = action.payload.total;
          state.serviceBookingsPage = action.payload.page;
          state.serviceBookingsLimit = action.payload.limit;
          state.serviceBookingsTotalPages = action.payload.totalPages;
        }
      )
