...
export default function BookingForm() {
  ...
  // const [numPets, setNumPets] = useState(0); // Pets are not allowed as per note
  ...
  const handleInfantsChange = (operation: 'increment' | 'decrement') => {
    setNumInfants(prev => {
      const newValue = operation === 'increment' ? prev + 1 : prev - 1;
      if (newValue < 0) return 0;
      if (newValue > 5) return 5; // As per image, 5 infants are shown
      return newValue;
    });
  };
  ...
  const guestDisplayValue = () => {
    ...
    // Pets are not allowed, so not adding them to display string.
    return text;
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <Card className="w-[400px] shadow-lg r0ounded-2xl ">
        <CardContent className="p-6">
          <div className="">
            <p className="text-xl font-semibold ">Thêm ngày để xem giá</p>
                          </div>
          ...
          <div className="rounded-xl border mb-4 overflow-hidden">
            ...
            <div className="flex flex-col">
                ...
                <PopoverTrigger asChild>
                  ...
                  <PopoverContent
                    align="start"
                    className="fixed left-[calc(50%+200px)] top-1/2 transform -translate-x-1/2 -translate-y-1/2 p-0 border-0 shadow-2xl rounded-2xl z-40 min-w-[600px] bg-white"
                      ...
                      <div className="flex flex-col h-full">
                        <Calendar
                          ...
                          classNames={{
                            ...
                            day_range_middle: "bg-white text-black", // Or bg-gray-100 if you prefer
                            day_outside: "text-gray-300",
                            day_disabled: "text-gray-300 opacity-50 cursor-not-allowed", // Added opacity and cursor
                            day_today: "", // Removed default today styling if not desired
                          }}
              ...
              <Popover open={guestPopoverOpen} onOpenChange={setGuestPopoverOpen} >
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className="w-full justify-between text-left font-normal rounded-none px-3 py-2 bg-white hover:bg-gray-50 text-black "
                  >
                    <span>{guestDisplayValue()}</span>
                    {guestPopoverOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[350px] p-0 bg-white border-none translate-y-[-3px] " align="start" >
          ...
          <Button className="w-full py-4 text-lg font-semibold rounded-full" style={{ background: 'linear-gradient(90deg, #e60073, #ff3385)' }}>
