...
export default function PropertyDetailView() {
  ...
  >('custom');
  const [activeTab, setActiveTab] = React.useState<'statistics' | 'rooms'>('statistics');
        ...
        <TabsContent value='statistics' className='mt-0'>
          ...
          <div className='flex flex-col md:flex-row gap-10 items-center mb-8 bg-white p-4 rounded-lg shadow-sm'>
            ...
            <div>
              ...
              <div className='flex gap-4 text-gray-500 mt-2'>
                ...
                {propertyDetail?.location?.district ? `, ${propertyDetail.location.district}` : ''}
                {propertyDetail?.location?.city ? `, ${propertyDetail.location.city}` : ''}
              </div>
              <div className='flex items-center gap-2 mt-2'>
                {renderStars(Math.round(data?.reviewStats?.averageRating || 0))}
                <span className='font-semibold text-lg'>{(data?.reviewStats?.averageRating ?? 0).toFixed(1)}</span>
                <span className='text-sm text-gray-500'>({data?.reviewStats?.totalReviews ?? 0} đánh giá)</span>
              </div>
            </div>
          </div>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-8'>
            <Card className='flex flex-col items-center gap-2 p-4 border-0 !border-none bg-white'>
              ...
              <div className='text-3xl font-bold'>{data?.occupancyStats?.occupancyRate?.toFixed(1) ?? '0.0'}%</div>
            ...
            <Card className='flex flex-col items-center gap-2 p-4 border-0 !border-none bg-white'>
              ...
              <div className='text-3xl font-bold'>{Math.round(data?.totalRevenue || 0).toLocaleString('vi-VN')}₫</div>
          ...
          {/* Revenue Chart */}
          <div className='mb-8'>
            <PropertyRevenueChart chartData={data?.chartData || []} dateRange={data?.dateRange} loading={loading} />
          </div>
          ...
          <div className='grid gap-6 lg:grid-cols-3 mb-8'>
                ...
                <BarChart
                  ...
                  data={
                    data?.reviewStats?.ratingDistribution
                      ? Object.entries(data.reviewStats.ratingDistribution).map(([rating, count]) => ({
                          rating: `${rating} sao`,
                          count: Number(count),
                        }))
                      : []
            ...
            <Card className='border-0 shadow-md bg-white'>
              ...
              <CardContent>
                <div className='space-y-3'>
                  {(data?.voucherStats?.topVouchers || []).slice(0, 5).map((voucher: any, index: number) => (
                    <div
                      key={voucher.voucherId}
                      className='flex items-center justify-between p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors'
                      onClick={() => navigate(`/admin/vouchers/${voucher.voucherId}/usage`)}>
                      <div className='flex items-center gap-3'>
                        <div className='w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center'>
                          <span className='text-sm font-bold text-purple-600'>{index + 1}</span>
                        </div>
                        <div>
                          <p className='font-medium text-sm'>{voucher.voucherCode}</p>
                          <p className='text-xs text-gray-500'>{voucher.usageCount} lần sử dụng</p>
                        </div>
                      </div>
                      <div className='text-right'>
                        <p className='font-bold text-sm text-purple-600'>
                          {voucher.totalDiscount?.toLocaleString('vi-VN') || 0}₫
                        </p>
                      </div>
                    </div>
                  ))}
            ...
            <Card className='border-0 shadow-md bg-white'>
              ...
              <CardContent>
                <div className='space-y-3'>
                  {(data?.serviceStats?.topServices || []).slice(0, 5).map((service: any, index: number) => (
                    <div
                      key={service.serviceId}
                      className='flex items-center justify-between p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors'
                      onClick={() => navigate(`/admin/services/${service.serviceId}/usage`)}>
                      <div className='flex items-center gap-3'>
                        ...
                        <div>
                          ...
                          <p className='text-xs text-gray-500'>{service.usageCount} lần sử dụng</p>
                        </div>
                      </div>
                      <div className='text-right'>
                        <p className='font-bold text-sm text-blue-600'>
                          {Math.round(service.totalRevenue || 0).toLocaleString('vi-VN')}₫
                        </p>
                      </div>
                    </div>
                  ))}
        ...
        <TabsContent value='rooms' className='mt-0'>
          {(() => {
            const roomsAsListings = Array.isArray(propertyRooms)
              ...
              : [];
            return <PropertyRoomsList listings={roomsAsListings} roomStatus={{}} propertyId={id} />;
          })()}
        </TabsContent>
      </Tabs>
    </div>
