...
import { Calendar, Users, Building2, CreditCard, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function PropertyBookingsList({ propertyId }: Props) {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const apiFilters = Object.fromEntries(Object.entries(filters).filter(([, v]) => v !== undefined && v !== ''));
      const params = {
        propertyId,
        page,
        limit,
    ...
    <div className='bg-white rounded-2xl border border-white/20 shadow-md overflow-hidden'>
      <div className='p-4 sm:p-6 border-b border-gray-200/50'>
        <div className='flex items-center gap-2'>
          ...
          <h2 className='text-lg sm:text-xl font-bold text-gray-900'>Danh sách Booking theo Homestay</h2>
        </div>
        {/* Filters */}
        <div className='flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 items-start sm:items-center mt-3'>
          <div>
            <select
              value={pendingFilters.status ? (pendingFilters.status as string) : 'all'}
              onChange={(e) => {
          ...
          <div>
            <select
              value={pendingFilters.paymentStatus ? (pendingFilters.paymentStatus as string) : 'all'}
              onChange={(e) => {
                const value = e.target.value;
          ...
          <div className='md:col-span-2'>
            <input
              ...
              value={pendingFilters.keyword ? (pendingFilters.keyword as string) : ''}
              onChange={(e) =>
                setPendingFilters((prev) => ({
                  ...prev,
            ...
            <Button
              onClick={() => {
                const processed = Object.fromEntries(
                  Object.entries(pendingFilters).map(([key, value]) => [key, value === 'all' ? undefined : value]),
                );
          ...
          <Table className='border-none min-w-[1200px]'>
            <TableHeader>
              <TableRow className='bg-gradient-to-r from-gray-50 to-gray-100 border-none'>
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-12'>STT</TableHead>
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-32'>
                  Khách hàng
                </TableHead>
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-32'>Phòng</TableHead>
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-32'>
                  HomeStay
                </TableHead>
                ...
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-16'>Khách</TableHead>
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-24'>
                  Trạng thái
                </TableHead>
                <TableHead className='text-center font-semibold text-gray-700 py-2 px-1 sm:px-2 w-24'>
              ...
              {currentBookings.length > 0 ? (
                currentBookings.map((b, idx) => {
                  ...
                  if (guestAny && typeof guestAny === 'object') {
                    ...
                    guestEmail = typeof guestAny.email === 'string' ? guestAny.email : guestEmail;
                  } else if (guestIdAny && typeof guestIdAny === 'object') {
                    guestName = guestIdAny.name || guestName;
                    guestEmail = typeof guestIdAny.email === 'string' ? guestIdAny.email : guestEmail;
                  } else if (typeof b.guest_name === 'string') {
                    guestName = b.guest_name || guestName;
                  } else if (typeof b.guest_name === 'object' && (b.guest_name as any)?.name) {
                    guestName = (b.guest_name as any).name || guestName;
                    guestEmail =
                      typeof (b.guest_name as any).email === 'string' ? (b.guest_name as any).email : guestEmail;
                  }
                  ...
                  if (typeof b.propertyId === 'object' && b.propertyId !== null) {
                    ...
                    propertyIdStr = ((b.propertyId as any)._id as string) || propertyId;
                  ...
                  const paymentStatus = getPaymentStatusVN(b.payment_status || b.paymentStatus);

                  return (
                    <TableRow
                      key={b._id}
                      className='hover:bg-gray-50/50 border-none cursor-pointer'
                      onClick={() => navigate(`/admin/bookings/${propertyIdStr}/${b._id}`)}>
                      <TableCell className='text-center py-2 px-1 sm:px-2'>
                        <div className='text-xs sm:text-sm'>{(page - 1) * limit + idx + 1}</div>
                      </TableCell>
                      <TableCell className='py-2 px-1 sm:px-2'>
                        <div className='flex items-center'>
                          ...
                          <div className='min-w-0 flex-1'>
                            <div className='font-semibold text-gray-900 text-xs sm:text-sm truncate'>{guestName}</div>
                            <div className='text-xs text-gray-500 truncate'>{guestEmail}</div>
                            {b.guest_phone && <div className='text-xs text-gray-400 truncate'>{b.guest_phone}</div>}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className='py-2 px-1 sm:px-2'>
                        ...
                        <div className='p-1 sm:p-2'>
                          <div className='flex items-center gap-2'>
                            ...
                            <span className='font-medium text-gray-900 text-xs sm:text-sm'>
                              {b.checkInDate ? new Date(b.checkInDate).toLocaleDateString('vi-VN') : '-'}
                            </span>
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className='py-2 px-1 sm:px-2'>
                        <div className='p-1 sm:p-2'>
                          <div className='flex items-center gap-2'>
                            ...
                            <span className='font-medium text-gray-900 text-xs sm:text-sm'>{b.guests}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className='py-2 px-1 sm:px-2'>
                        <div className='p-1 sm:p-2'>
                          <Badge className={`${bookingStatus.color} font-medium text-xs`}>{bookingStatus.label}</Badge>
                        </div>
                      </TableCell>
                      <TableCell className='py-2 px-1 sm:px-2'>
                        <div className='p-1 sm:p-2'>
                          <Badge className={`${paymentStatus.color} font-medium text-xs`}>{paymentStatus.label}</Badge>
                        </div>
                      </TableCell>
                      <TableCell className='py-2 px-1 sm:px-2'>
                        <div className='p-1 sm:p-2'>
