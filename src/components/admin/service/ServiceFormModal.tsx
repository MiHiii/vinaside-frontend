...
const ServiceFormModal = ({ initialValues, loading, onSubmit, onClose, isEdit }: Props) => {
  ...
  const { register, handleSubmit, reset, formState: { errors }, setValue, watch } = useForm<ServiceFormValues>({
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-fadeIn">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="px-6 py-6 space-y-5">
          <div>
            ...
            <input {...register("name")} placeholder="Nhập tên dịch vụ" className={`border rounded-md px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-primary/50 ${errors.name ? 'border-red-500' : 'border-gray-300'}`} />
            {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Mô tả</label>
            <textarea {...register("description")} placeholder="Mô tả dịch vụ (không bắt buộc)" className="border rounded-md px-3 py-2 w-full min-h-[60px] focus:outline-none focus:ring-2 focus:ring-primary/50 border-gray-300" />
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              ...
              <input {...register("unit")} placeholder="Đơn vị (VD: /ngày, /lần)" className={`border rounded-md px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-primary/50 ${errors.unit ? 'border-red-500' : 'border-gray-300'}`} />
            ...
            <div className="flex-1">
              ...
              <input type="number" {...register("default_price")} placeholder="Giá" className={`border rounded-md px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-primary/50 ${errors.default_price ? 'border-red-500' : 'border-gray-300'}`} />
