...
const StaffBookingModal: React.FC<StaffBookingModalProps> = ({
  ...
  console.log("🌟 StaffBookingModal rendered, isOpen:", isOpen);
  ...
  const loadBookedDates = async () => {
    ...
    try {
      ...
      if (checkIn && checkOut) {
        ...
        const bookedDatesYMD = bookedDatesArray.map((date: Date) =>
  ...
  const resetForm = () => {
    setFormData({
      ...
      final_amount: undefined,
      skip_availability_check: false,
      // Reset payment integration fields
      create_payment_url: false,
      payment_method_choice: undefined,
      payment_type: "full",
      payment_description: undefined,
      payment_return_url: undefined,
      payment_notify_url: undefined,
  ...
  const handleSubmit = async () => {
    ...
    const submitBooking = async () => {
      ...
      try {
        ...
        // Xóa các payment fields nếu không cần thiết
        if (!payload.create_payment_url) {
          delete payload.payment_method_choice;
          delete payload.payment_type;
          delete payload.payment_description;
          delete payload.payment_return_url;
          delete payload.payment_notify_url;
          delete payload.payment_amount;
        } else {
          // Chỉ giữ lại các field cần thiết và có giá trị
          if (
            !payload.payment_description ||
            payload.payment_description.trim() === ""
          ) {
            delete payload.payment_description;
          }
          if (
            !payload.payment_notify_url ||
            payload.payment_notify_url.trim() === ""
          ) {
            delete payload.payment_notify_url;
          }
          ...
          // Validate payment_return_url trước khi gửi
          if (
            !payload.payment_return_url ||
            payload.payment_return_url.trim() === ""
          ) {
            delete payload.payment_return_url;
          } else {
            try {
              const testUrl = new URL(payload.payment_return_url);
              console.log("URL validation passed:", testUrl.href);
            } catch (e) {
              console.error("URL validation failed, removing field:", e);
              delete payload.payment_return_url;
            }
          }
        }
        ...
        // Logic cho VNPay: Set payment_type dựa trên payment_status được chọn
        console.log(
          "DEBUG: Before VNPay logic - payment_method:",
          formData.payment_method,
          "create_payment_url:",
          payload.create_payment_url
        );
        console.log("🔥 === CHECKING VNPAY CONDITIONS ===");
        console.log(
          "formData.payment_method === 'vnpay':",
          formData.payment_method === "vnpay"
        );
        console.log("payload.create_payment_url:", payload.create_payment_url);

        if (formData.payment_method === "vnpay" && payload.create_payment_url) {
          console.log("✅ === VNPay Payment Type Logic ACTIVATED ===");
          console.log("Payment Status:", payload.payment_status);
          console.log("Payment Method:", formData.payment_method);
          console.log("Create Payment URL:", payload.create_payment_url);

          // Set payment_type dựa trên payment_status
          if (payload.payment_status === "paid") {
            payload.payment_type = "full"; // Thanh toán toàn bộ
            payload.deposit_paid_amount = 0; // Chưa thanh toán, sẽ thanh toán qua VNPay
            console.log("Set payment_type to: full (100%)");
          } else if (payload.payment_status === "partially_paid") {
            payload.payment_type = "deposit"; // Thanh toán 1 phần (cọc)
            payload.deposit_paid_amount = 0; // Chưa thanh toán, sẽ thanh toán qua VNPay
            console.log("Set payment_type to: deposit (50%)");
          } else {
            payload.payment_type = "full"; // Mặc định thanh toán toàn bộ
            payload.deposit_paid_amount = 0;
            console.log("Set payment_type to: full (default)");
          }
        ...
        // Sử dụng tích hợp mới với backend đã fix validation
        const useVnpayIntegration = true;

        if (payload.payment_method === "vnpay" && useVnpayIntegration) {
          // Sử dụng endpoint tích hợp mới
          // payload.create_payment_url = true; // Already set early above
          payload.payment_method_choice = "vnpay";
          // DON'T override payment_type here - it's already set correctly above
          payload.payment_description = "Admin tạo booking với VNPay";
          ...
          try {
            // Lấy protocol, hostname và port từ window.location
            ...
            returnUrl = `${protocol}//${hostname}${port}/admin/bookings?payment=success`;

            // Validate URL
            const urlObj = new URL(returnUrl);
            console.log("Generated URL object:", {
            ...
            payload.payment_return_url = urlObj.href;
            console.log(
              "Final payment return URL:",
              payload.payment_return_url
            );
          } catch (urlError) {
            console.error("Error generating URL, using fallback:", urlError);
            payload.payment_return_url =
              "http://localhost:5173/admin/bookings?payment=success";
          }
        }
        ...
        console.log("payment_type:", payload.payment_type);
        console.log("create_payment_url:", payload.create_payment_url);
        ...
        // Check response structure for integrated payment
        if (bookingResponse.data?.data?.paymentUrl?.paymentUrl) {
          // New integrated response - has both booking and payment URL
          const { booking, paymentUrl } = bookingResponse.data.data;
          console.log("Integrated response received:", { booking, paymentUrl });

          toast.success("Tạo booking thành công!");

          if (paymentUrl?.paymentUrl) {
            console.log("Redirecting to VNPay:", paymentUrl.paymentUrl);
            onSuccess(); // Call before redirect
            onClose();
            // Redirect to VNPay
            window.location.href = paymentUrl.paymentUrl;
            return;
          }
        } else if (bookingResponse.data?.data?.booking) {
          // New integrated response but no payment URL
          toast.success("Tạo booking thành công!");
          if (bookingResponse.data.data.error) {
            toast.warning(bookingResponse.data.data.error);
          }
        } else {
          // Old response format - booking created without payment URL
          const bookingId =
            bookingResponse.data?.data?._id || bookingResponse.data?._id;
          toast.success("Tạo booking thành công!");

          // Fallback: Use old method if VNPay was selected but no payment URL received
          if (
            ...
            try {
              console.log(
                "Creating VNPay payment for booking (fallback):",
                bookingId
              );
              const paymentResponse = await api.post(
                `/bookings/${bookingId}/payment/staff`,
                {
                  ...
                  note: "Admin tạo booking với VNPay (fallback)",
                }
              );
              ...
              if (paymentUrl) {
                console.log("Redirecting to VNPay (fallback):", paymentUrl);
                onSuccess();
                onClose();
                ...
                console.error(
                  "No payment URL received (fallback):",
              ...
              console.error(
                "Error creating VNPay payment (fallback):",
                paymentError
              );
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            const allServices = services.map((service) => ({
                              ...
                              quantity: SERVICE_CONSTANTS.MIN_QUANTITY,
                            }));
                            setFormData((prev) => ({
                              ...prev,
                      ...
                      services.map((service) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {service.allow_quantity && isSelected && (
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    ...
                                    if (
                                      ...
                                      const newServices = formData.services.map(
                                        (s) =>
                                ...
                                {existingService?.quantity >=
                                  ...
                                  <span className="text-xs text-gray-500">
                                    {SERVICE_MESSAGES.QUANTITY_LIMIT_HINT}
                                  </span>
                                )}
                              </div>
                            )}
