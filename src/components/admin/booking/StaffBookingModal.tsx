...
const StaffBookingModal: React.FC<StaffBookingModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (selectedListing && nights > 0 && checkIn && checkOut) {
      ...
      const subtotalBeforeDiscount = totalPrice + servicesTotal + additionalCosts;
  ...
  const loadBookedDates = async () => {
    ...
    try {
      ...
      const bookedDatesArray = dates.map((dateStr: string) => new Date(dateStr));
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (allUsersRes.data && allUsersRes.data.data && Array.isArray(allUsersRes.data.data)) {
            // Lọc chỉ lấy users có role guest
            guestsData = allUsersRes.data.data.filter(
              (user: { role?: string }) => user.role === 'guest' || user.role === 'Guest',
            );
  ...
  const handleSubmit = async () => {
    ...
    const submitBooking = async () => {
      ...
      try {
        ...
        const bookingResponse = await api.post('/bookings/staff/create', payload);
    ...
    showConfirm(
      ...
      `Bạn có chắc chắn muốn tạo booking cho khách hàng "${formData.guest_name}" từ ${format(
        checkIn!,
        'dd/MM/yyyy',
      )} đến ${format(checkOut!, 'dd/MM/yyyy')}?`,
      submitBooking,
      'Tạo booking',
    );
  };
                        ...
                        <div className='flex items-center gap-4'>
                          ...
                          <div className='text-left'>
                            {(() => {
                              ...
                              if (formData.guestId && formData.guestId !== '__new__') {
                              ...
                              console.log('formData.guestId:', formData.guestId);
                              console.log('formData.guest_name:', formData.guest_name);
                              console.log('formData.guest_email:', formData.guest_email);
                              console.log('formData.guest_phone:', formData.guest_phone);
                      ...
                      <Command className='rounded-2xl'>
                            ...
                            <div className='flex gap-2'>
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    await loadGuests();
                                    toast.success('Đã tải lại danh sách guests');
                                  } catch {
                                    toast.error('Không thể tải lại danh sách guests');
                                  }
                                }}
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  console.log('Guests length:', guests.length);
                                  console.log('Search value:', guestSearchValue);
                                  if (guests.length > 0) {
                                    console.log('First guest:', guests[0]);
                                    console.log('Guest name:', guests[0].name);
                                    console.log('Guest email:', guests[0].email);
                                  }
                                  toast.info(`Có ${guests.length} guests trong danh sách`);
                                }}
                        ...
                        <CommandList
                          ...
                          {(() => {
                            ...
                            console.log('guests length:', guests.length);
                            console.log('filteredGuests length:', filteredGuests.length);
                            if (filteredGuests.length > 0) {
                              console.log('First filtered guest:', filteredGuests[0]);
                            }

                            // Nếu không có kết quả tìm kiếm và có search value
                            if (filteredGuests.length === 0 && guestSearchValue) {
                              return (
                                <CommandEmpty>
                            ...
                            console.log('Will show guests section, filteredGuests.length:', filteredGuests.length);
                            return (
                              <>
                          ...
                          <div
                            ...
                            <div className='flex-1'>
                              ...
                              <div className='text-sm text-gray-500'>
                                ...
                                {service.allow_quantity && (
                                  <span className='ml-1 text-green-600 font-medium'>• Có thể chọn số lượng</span>
                                )}
                              </div>
                            </div>
                            ...
                            {service.allow_quantity && isSelected && (
                              <div className='flex items-center gap-2'>
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    const currentService = formData.services.find((s) => s.serviceId === service._id);
                                    if (currentService && currentService.quantity > SERVICE_CONSTANTS.MIN_QUANTITY) {
                                      const newServices = formData.services.map((s) =>
                                        s.serviceId === service._id ? { ...s, quantity: s.quantity - 1 } : s,
                                      );
                                      setFormData((prev) => ({
                                        ...prev,
                                  ...
                                  disabled={existingService?.quantity <= SERVICE_CONSTANTS.MIN_QUANTITY}
                                ...
                                <span className='text-sm font-medium min-w-[20px] text-center'>
                                  {existingService?.quantity || SERVICE_CONSTANTS.MIN_QUANTITY}
                                </span>
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    const currentService = formData.services.find((s) => s.serviceId === service._id);
                                    if (currentService && currentService.quantity < SERVICE_CONSTANTS.MAX_QUANTITY) {
                                      const newQuantity = currentService.quantity + 1;
                                      const newServices = formData.services.map((s) =>
                                        s.serviceId === service._id ? { ...s, quantity: newQuantity } : s,
                                      );
                                      setFormData((prev) => ({
                                        ...prev,
                                        services: newServices,
                                  ...
                                  disabled={existingService?.quantity >= SERVICE_CONSTANTS.MAX_QUANTITY}
                                  className='w-6 h-6 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed'>
                                  +
                                </button>
                                {existingService?.quantity >= SERVICE_CONSTANTS.MAX_QUANTITY && (
                            ...
                            <div className='text-right'>
                              <div className='font-medium text-red-600'>
                                ...
                                existingService.quantity > SERVICE_CONSTANTS.MIN_QUANTITY
                                  ? (service.default_price * existingService.quantity).toLocaleString()
                              ...
                              {isSelected &&
                                ...
                                existingService.quantity > SERVICE_CONSTANTS.MIN_QUANTITY && (
                                  <div className='text-xs text-gray-500'>
                                    {existingService.quantity} x {service.default_price?.toLocaleString()}₫
                                  </div>
                                )}
                            </div>
                              ...
                              onCheckedChange={(checked) => {
                                if (checked) {
                                  ...
                                  setFormData((prev) => ({
                                    ...
                                    services: [
                                      ...prev.services,
                                      { serviceId: service._id, quantity: SERVICE_CONSTANTS.MIN_QUANTITY },
                                    ],
                                  }));
                                } else {
                                  // Remove service
                  ...
                  <div className='bg-gray-50 p-6 rounded-xl border border-gray-200'>
                    ...
                    <div className='space-y-3 text-sm bg-white rounded-lg border border-gray-100 p-4'>
                      ...
                      <div className='flex justify-between items-center py-2'>
                        <span className='text-gray-600'>Giá phòng ({nights || 0} đêm)</span>
                        <span className='font-medium text-gray-800'>
                          {selectedListing
                            ? (
                                (formData.price_per_night || selectedListing.price_per_night) * (nights || 0)
                              ).toLocaleString()
                            : '0'}
                          ₫
                        </span>
                          ...
                          return weekendNights > 0 ? (
                            <div className='flex justify-between items-center py-2'>
                              <span className='text-gray-600'>
                                Phí cuối tuần ({weekendNights} đêm - {selectedListing.weekend_surcharge_percent}%)
                              </span>
                              <span className='font-medium text-orange-600'>+{weekendSurcharge.toLocaleString()}₫</span>
                            </div>
                          ) : null;
                        })()}
                              ...
                              return (
                                <div key={index} className='flex justify-between items-center text-xs text-gray-600'>
                                  ...
                                  <span className='font-medium'>
                                    {(serviceData.default_price * service.quantity).toLocaleString()}₫
                                  </span>
                                </div>
                              );
                            })}
                      ...
                      {formData.voucherCode &&
                        (() => {
                          ...
                          const basePrice = selectedListing
                            ? (formData.price_per_night || selectedListing.price_per_night) * nights
                            : 0;
                          ...
                          const subtotal = basePrice + weekendSurcharge + servicesTotal + formData.additionalCost;
                          const discountAmount = subtotal * (selectedVoucher.discount_percent / 100);

                          return (
                            <div className='flex justify-between items-center py-2'>
                              <span className='text-gray-600'>
                                Giảm giá ({formData.voucherCode} - {selectedVoucher.discount_percent}%)
                              </span>
                          ...
                          return (
                            <>
                              ...
                              <div className='flex justify-between items-center py-2'>
                                ...
                                <span className='font-medium text-gray-600'>+{taxAmount.toLocaleString()}₫</span>
                              </div>
                            </>
                          );
                        })()}
