...
import { fetchAdminBookingDetail, updateAdminBookingStatus, updateAdditionalCost } from '@/store/slices/bookingSlice';
...
const BookingDetail: React.FC<{
  ...
  const { adminBookingDetail, loading, error } = useSelector((state: RootState) => state.booking);
  ...
  useEffect(() => {
    if (propertyId && bookingId) dispatch(fetchAdminBookingDetail({ propertyId, id: bookingId }));
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Có lỗi xảy ra khi xác nhận booking';
  ...
  const handleCompleteBooking = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Có lỗi xảy ra khi hoàn thành booking';
      toast.error(errorMessage);
    }
  };
  ...
  if (
    ...
    (typeof booking.vnpay_pay_date === 'string' || typeof booking.vnpay_pay_date === 'number')
  ) {
  ...
  function hasListingFields(obj: unknown): obj is { _id?: string; title?: string; address?: string } {
  ...
  const getServiceIcon = (service: { service_name: string; service_id?: string }): React.ReactElement => {
    ...
    const matchedService = services.find((s) => {
      ...
      return (
        serviceObj.name?.toLowerCase() === service.service_name.toLowerCase() || serviceObj._id === service.service_id
      );
    });
    ...
    if (
      ...
      return (
        <img src={matchedService.icon_url as string} alt={service.service_name} className='w-4 h-4 object-contain' />
      );
    }
    ...
    if (name.includes('coffee') || name.includes('cà phê')) return <Coffee className='w-4 h-4' />;
    if (name.includes('parking') || name.includes('xe')) return <Car className='w-4 h-4' />;
    if (name.includes('breakfast') || name.includes('ăn sáng')) return <Utensils className='w-4 h-4' />;
    if (name.includes('gym') || name.includes('thể dục')) return <Dumbbell className='w-4 h-4' />;
    if (name.includes('baby') || name.includes('trẻ em')) return <Baby className='w-4 h-4' />;
    if (name.includes('pet') || name.includes('thú cưng')) return <Dog className='w-4 h-4' />;
    if (name.includes('camera') || name.includes('máy ảnh')) return <Camera className='w-4 h-4' />;
    if (name.includes('music') || name.includes('nhạc')) return <Music className='w-4 h-4' />;
    if (name.includes('tv') || name.includes('tivi')) return <Tv className='w-4 h-4' />;
    if (name.includes('ac') || name.includes('điều hòa')) return <Snowflake className='w-4 h-4' />;
    if (name.includes('heater') || name.includes('sưởi')) return <Sun className='w-4 h-4' />;
    if (name.includes('electric') || name.includes('điện')) return <Zap className='w-4 h-4' />;
              ...
              <div className='flex gap-2'>
                ...
                {(status === BookingStatus.PENDING ||
                  ...
                  <Button
                    ...
                    title={status === BookingStatus.COMPLETED ? 'Booking đã hoàn thành' : ''}>
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6'>
              <div className='flex items-center mb-4'>
                {getGuestAvatar(guest) ? (
                  <img src={getGuestAvatar(guest)} alt='avatar' className='w-12 h-12 rounded-full object-cover' />
                ) : (
              ...
              <div className='p-6'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  ...
                  <div>
                    ...
                    <div className='mt-1'>
                      <Badge className={`${paymentStatus.color} text-xs`}>{paymentStatus.label as string}</Badge>
                    </div>
                  </div>
                  <div>
                        ...
                        {selected_services.map((service, index) => (
                          <tr key={index}>
                            <td className='px-4 py-3 whitespace-nowrap'>
                              <div className='flex items-center'>
                                <div className='w-4 h-4'>{getServiceIcon(service)}</div>
              ...
              <div className='p-6'>
                <div className='space-y-3 text-sm bg-white rounded-lg border border-gray-100 p-4'>
                  ...
                  {((services_total_amount as number) || 0) > 0 && (
                    <div className='space-y-2'>
                      <div className='flex justify-between items-center py-2'>
                        <span className='text-gray-600 font-medium'>Dịch vụ bổ sung</span>
                      ...
                      {/* Chi tiết dịch vụ */}
                      {adminBookingDetail?.selected_services && adminBookingDetail.selected_services.length > 0 && (
                          ...
                          {adminBookingDetail.selected_services.map(
                            (
                              ...
                              <div key={index} className='flex justify-between items-center text-xs text-gray-600'>
                                ...
                                <span className='font-medium'>{(service.total_price || 0).toLocaleString()}₫</span>
                              </div>
                            ),
                          )}
                        </div>
                  ...
                  {((discount_amount as number) || 0) > 0 && (
                    <div className='flex justify-between items-center py-2'>
                      <span className='text-gray-600'>
                        Giảm giá {adminBookingDetail?.voucher_code ? `(${adminBookingDetail.voucher_code})` : ''}
                      </span>
                  ...
                  <div className='border-t border-gray-200 pt-3 mt-3'>
                    ...
                    <div className='flex justify-between items-center py-2'>
                      ...
                      <span className='font-medium text-gray-800'>
                        {((amount_after_discount as number) || (subtotal_amount as number) || 0).toLocaleString()}₫
                      </span>
                    </div>
                ...
                <div className='p-6'>
                  {booking.cancellationDetails ? (
                        ...
                        <div className='bg-gradient-to-r from-red-50 to-orange-50 rounded-lg p-6 border-2 border-red-200 shadow-sm'>
                          <div className='flex items-start gap-4'>
                            ...
                            <div className='flex-1'>
                              <h3 className='text-base font-semibold text-red-900 mb-3'>Lý do hủy phòng</h3>
                            ...
                            {booking.cancellationDetails.accountName && (
                              <div className='bg-green-50 rounded-lg p-4 border border-green-200'>
                                <div className='flex items-center gap-2 mb-2'>
                                  ...
                                  <span className='text-sm font-semibold text-green-900'>Tên chủ tài khoản</span>
                                </div>
                            ...
                            {booking.cancellationDetails.bankName && (
                              <div className='bg-blue-50 rounded-lg p-4 border border-blue-200'>
                                <div className='flex items-center gap-2 mb-2'>
                                  ...
                                  <span className='text-sm font-semibold text-blue-900'>Ngân hàng</span>
                                </div>
                            ...
                            {booking.cancellationDetails.accountNumber && (
                              <div className='bg-purple-50 rounded-lg p-4 border border-purple-200'>
                                <div className='flex items-center gap-2 mb-2'>
                                  ...
                                  <span className='text-sm font-semibold text-purple-900'>Số tài khoản</span>
                                </div>
                            ...
                            {booking.cancellationDetails.refundMethod && (
                              <div className='bg-orange-50 rounded-lg p-4 border border-orange-200'>
                                <div className='flex items-center gap-2 mb-2'>
                                  ...
                                  <span className='text-sm font-semibold text-orange-900'>Phương thức hoàn tiền</span>
                                </div>
                                <p className='text-sm font-medium text-orange-800'>
                                  {booking.cancellationDetails.refundMethod === 'bank_transfer' &&
                                    'Chuyển khoản ngân hàng'}
                                  {booking.cancellationDetails.refundMethod === 'cash' && 'Tiền mặt'}
                                  {booking.cancellationDetails.refundMethod === 'credit_card' && 'Thẻ tín dụng'}
                                  {booking.cancellationDetails.refundMethod === 'other' && 'Khác'}
                                  {!['bank_transfer', 'cash', 'credit_card', 'other'].includes(
                                    booking.cancellationDetails.refundMethod,
                                  ) && booking.cancellationDetails.refundMethod}
                                </p>
                              </div>
                      ...
                      {booking.cancellationDetails.refundNote && (
                        <div className='bg-yellow-50 rounded-lg p-4 border border-yellow-200'>
                          <div className='flex items-start gap-3'>
                            ...
                            <div className='flex-1'>
                              <h3 className='text-sm font-semibold text-yellow-900 mb-2'>Ghi chú hoàn tiền</h3>
                        ...
                        <div className='bg-gray-50 rounded-lg p-4 border border-gray-200'>
                          <div className='flex items-center gap-2'>
                            ...
                            <span className='text-sm font-medium text-gray-700'>
                              Cập nhật lần cuối:{' '}
                              {new Date(booking.cancellationDetails.cancellationDetailsUpdatedAt).toLocaleString(
                                'vi-VN',
                              )}
                            </span>
                          </div>
                    ...
                    <div className='text-center py-8'>
                      ...
                      <h3 className='text-lg font-medium text-gray-900 mb-2'>Chưa có thông tin hủy phòng</h3>
                      <p className='text-gray-500 mb-6 max-w-md mx-auto'>
                        Khách hàng chưa cung cấp thông tin chi tiết về việc hủy phòng hoặc thông tin chưa được cập nhật.
                      </p>
                      <Button
                        onClick={() => setShowEditCancellationModal(true)}
