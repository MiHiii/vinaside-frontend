...
export default function DashboardContent() {
      ...
      if (isStaff && user?._id) {
        try {
          ...
          if (response.success && response.data && response.data.length > 0) {
            ...
            const staffPropertyIds = response.data.map((item: { propertyId?: { _id: string } }) => item.propertyId?._id).filter(Boolean);
          ...
          if (response.success && response.data && response.data.length > 0) {
            ...
            if (staffPropertyIds.length > 0) {
              ...
              if (staffPropertyIds.length > 1) {
                ...
                console.log('Sending all property IDs for staff:', allPropertyIds);
                apiParams.propertyId = allPropertyIds;
              } else {
                // Single property
                const selectedPropertyId = staffPropertyIds[0];
                console.log('Selected property ID for staff:', selectedPropertyId);
  ...
  if (isStaff) {
    ...
    console.log('Real-time data should be filtered by property:', selectedPropertyId);
  console.log('Expected: Data should be aggregated from all assigned properties');
  }

  // Convert booking status data for pie chart
  const bookingStatusData = statistics?.overview?.bookingsByStatus ? 
    Object.entries(statistics.overview.bookingsByStatus).map(([status, count]) => ({
      name: status,
      value: count
    })) : [];
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">
            {isAdmin ? "Bảng điều khiển quản trị viên" : isStaff ? "Bảng điều khiển nhân viên" : "Bảng điều khiển"}
          </h1>
          <p className="text-gray-600 mt-1">
            {isAdmin ? "Quản lý tổng quan hệ thống" : "Quản lý properties được assign"}
          </p>
        </div>
        ...
        <Card className="border-0 shadow-lg bg-gradient-to-br from-orange-50 to-orange-100">
          ...
          <CardContent>
            ...
            <p className="text-xs text-orange-600 mt-1">
              {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(statistics?.overview?.totalRevenue || 0)}
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <Card className="border-0 shadow-lg">
            ...
            <CardContent>
              <div className="space-y-3">
                {(statistics.financial?.topPropertiesByRevenue || []).slice(0, 5).map((property, index) => (
                  <div key={property.propertyId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-sm">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(property.revenue)}
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between items-center p-3 bg-indigo-50 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-indigo-600">{statistics.overview?.totalUsers || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                  <span className="text-sm font-medium">Khách</span>
                  <span className="text-lg font-bold text-green-600">{statistics.overview?.usersByRole?.guest || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                  <span className="text-sm font-medium">Nhân viên</span>
                  <span className="text-lg font-bold text-blue-600">{statistics.overview?.usersByRole?.staff || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                  <span className="text-sm font-medium">Quản trị viên</span>
                  <span className="text-lg font-bold text-purple-600">{statistics.overview?.usersByRole?.admin || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-orange-50 rounded-lg">
                  <span className="text-sm font-medium">Users mới (30 ngày)</span>
                  <span className="text-lg font-bold text-orange-600">{statistics.overview?.newUsersLast30Days || 0}</span>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between items-center p-3 bg-emerald-50 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-emerald-600">{statistics.overview?.totalProperties || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                  <span className="text-sm font-medium">Properties hoạt động</span>
                  <span className="text-lg font-bold text-green-600">{statistics.overview?.activeProperties || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                  <span className="text-sm font-medium">Properties đang chờ</span>
                  <span className="text-lg font-bold text-yellow-600">{statistics.overview?.propertiesByStatus?.pending || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                  <span className="text-sm font-medium">Tổng Listings</span>
                  <span className="text-lg font-bold text-blue-600">{statistics.overview?.totalListings || 0}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                  <span className="text-sm font-medium">Listings hoạt động</span>
                  <span className="text-lg font-bold text-purple-600">{statistics.overview?.activeListings || 0}</span>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-green-600">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(statistics.financial?.totalRevenue || 0)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                  <span className="text-sm font-medium">Net Revenue</span>
                  <span className="text-lg font-bold text-blue-600">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(statistics.financial?.netRevenue || 0)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                  <span className="text-sm font-medium">Phí dịch vụ</span>
                  <span className="text-lg font-bold text-yellow-600">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(statistics.financial?.totalServiceFees || 0)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
                  <span className="text-sm font-medium">Chiết khấu Voucher</span>
                  <span className="text-lg font-bold text-red-600">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(statistics.financial?.totalVoucherDiscount || 0)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                  <span className="text-sm font-medium">Doanh thu dịch vụ</span>
                  <span className="text-lg font-bold text-purple-600">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(statistics.financial?.totalServicesRevenue || 0)}
                  </span>
                </div>
              </div>
            </CardContent>
      ...
      {statistics && (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="border-0 shadow-lg">
            ...
            <CardContent>
              <div className="space-y-3">
                {(statistics.customers?.topCustomers || []).slice(0, 5).map((customer, index) => (
          ...
          <Card className="border-0 shadow-lg">
            ...
            <CardContent>
              <div className="space-y-3">
                {(statistics.performance?.topServices || []).slice(0, 5).map((service, index) => (
                  <div key={service.serviceId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{service.usageCount} uses</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-sm">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(service.totalRevenue)}
                      </p>
                    </div>
                  </div>
                ))}
        ...
        <div className="grid gap-6 lg:grid-cols-4">
          <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100">
            ...
            <CardContent>
              ...
              <p className="text-xs text-blue-600 mt-1">Tổng {statistics.overview?.totalReviews || 0} reviews</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-lg bg-gradient-to-br from-orange-50 to-orange-100">
            ...
            <CardContent>
              <p className="text-3xl font-bold text-orange-700">
                {statistics.performance?.averageServicesPerBooking?.toFixed(1) || 0}
              </p>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  <p className="text-2xl font-bold text-blue-600">{realTimeData.activeBookings}</p>
                  <p className="text-sm text-blue-600">Active Bookings</p>
                </div>
                <div className="text-center p-4 bg-yellow-50 rounded-lg">
                  <p className="text-2xl font-bold text-yellow-600">{realTimeData.pendingBookings}</p>
                  <p className="text-sm text-yellow-600">Pending Bookings</p>
                </div>
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  <p className="text-2xl font-bold text-green-600">{realTimeData.recentMessages}</p>
                  <p className="text-sm text-green-600">Recent Messages</p>
                </div>
                <div className="text-center p-4 bg-purple-50 rounded-lg">
                  <p className="text-2xl font-bold text-purple-600">{realTimeData.recentReviews}</p>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-sm">Điểm đánh giá trung bình</p>
                      <p className="text-xs text-gray-500">{statistics?.overview?.averageRating?.toFixed(1) || 0}/5</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-sm">{statistics?.overview?.totalReviews || 0}</p>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-sm">{statistics?.overview?.totalMessages || 0}</p>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-sm">Vouchers đang hoạt động</p>
                      <p className="text-xs text-gray-500">Active vouchers</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-sm">{statistics?.overview?.activeVouchers || 0}</p>
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              <Button className="p-4 h-auto flex-col gap-2" variant="outline">
                <Building2 className="w-6 h-6" />
                <span>Quản lý Properties</span>
              </Button>
              <Button className="p-4 h-auto flex-col gap-2" variant="outline">
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              <Button className="p-4 h-auto flex-col gap-2" variant="outline">
                <Building2 className="w-6 h-6" />
                <span>Quản lý Properties được assign</span>
              </Button>
              <Button className="p-4 h-auto flex-col gap-2" variant="outline">
