...
export default function Listings() {
  ...
  console.log("🔄 Listings component re-render:", {
    isAdmin,
    isStaff,
    staffPropertiesLoaded,
  });
      ...
      if (isStaff && !staffPropertiesLoaded && mounted) {
        try {
          ...
          if (userId) {
            ...
            const propertyIds = staffAssignments.map(
              (assignment: AssignmentData) => assignment.propertyId._id
            );
  ...
  useEffect(() => {
    if (isAdmin) {
      console.log(
        "📞 Calling fetchAdminListings for admin with params:",
        memoizedFilters
      );
      console.log(
        "📄 Current page:",
      ...
      // Clear old data before fetching new page
  ...
  useEffect(() => {
    if (isStaff && staffPropertiesLoaded && staffPropertyIds.length > 0) {
      console.log(
        "📞 Calling fetchAdminListings for staff with params:",
        memoizedFilters
      );
      console.log(
        "📄 Current page:",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Quản lý tất cả phòng và bất động sản
          </p>
        </div>
            ...
            <Table className="border-none">
              <TableHeader>
                <TableRow className="border-none bg-gray-50 dark:bg-gray-700">
                  <TableHead className="border-none text-gray-700 dark:text-gray-200 font-semibold py-4 px-6 text-sm uppercase tracking-wide">
                    Ảnh
                  </TableHead>
                  <TableHead className="border-none text-gray-700 dark:text-gray-200 font-semibold py-4 px-6 text-sm uppercase tracking-wide">
                    Tiêu đề
                  </TableHead>
                  <TableHead className="border-none text-gray-700 dark:text-gray-200 font-semibold py-4 px-6 text-sm uppercase tracking-wide text-right">
                    Giá/đêm
                  </TableHead>
                  <TableHead className="border-none text-gray-700 dark:text-gray-200 font-semibold py-4 px-6 text-sm uppercase tracking-wide text-center">
                    Phí cuối tuần
                  </TableHead>
                  <TableHead className="border-none text-gray-700 dark:text-gray-200 font-semibold py-4 px-6 text-sm uppercase tracking-wide">
                    HomeStay
                  ...
                  <TableHead className="text-right border-none text-gray-700 dark:text-gray-200 font-semibold py-4 px-6 text-sm uppercase tracking-wide">
                    Thao tác
                  </TableHead>
                </TableRow>
              </TableHeader>
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${
                          listing.status === "active"
                            ...
                            : "bg-red-400 text-gray-800 dark:bg-gray-900 dark:text-gray-200"
                        }`}
                      >
                        {listing.status === "active"
                          ? "Hoạt động"
            ...
            {listings.length === 0 && (
              <div className="text-center py-12">
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                  Hãy thêm listing đầu tiên để bắt đầu
                </div>
                <Link to="/admin/listings/create">
                  <Button className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm hover:shadow-md transition-all duration-200">
                    <Plus className="w-4 h-4 mr-2" />
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="bg-white border shadow-lg rounded-lg p-8">
          <DialogHeader>
            ...
            <DialogDescription>
              Bạn có chắc chắn muốn xóa listing "{selectedListing?.title}"? Hành
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button
              ...
              onClick={() =>
                selectedListing?._id && handleDelete(selectedListing._id)
              }
            >
              Xóa
            </Button>
