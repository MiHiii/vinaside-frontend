...
interface BookingSummaryProps {
  ...
  propertyId: string;
  bookedDates: Date[];
  onSaveBookingInfo: (data: { dateRange: DateRange | undefined; guests: { adults: number; infants: number } }) => void;
...
const BookingSummary: React.FC<BookingSummaryProps> = ({
  ...
  const [internalPaymentType, internalSetPaymentType] = useState<'full' | 'deposit'>('full');
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!listing.propertyId || typeof listing.propertyId !== 'object' || !listing.propertyId._id) {
      ...
      if (
        ...
        toast.error('Đã có lỗi xảy ra khi thanh toán! Mã voucher đã bị vô hiệu hóa', {
          style: { color: '#dc2626' },
        });
      }
    } finally {
  ...
  const discount = selectedVoucher ? Math.round((totalAmount * selectedVoucher.discount_percent) / 100) : 0;
    ...
    <Card className='sticky top-6 rounded-xl border border-gray-200 shadow-sm'>
      <CardContent className='p-6 space-y-6'>
        ...
        <div className='space-y-2'>
          <label htmlFor='specialRequests' className='block font-semibold text-lg'>
            Yêu cầu đặc biệt
          </label>
          <textarea
            id='specialRequests'
        ...
        <div className='flex gap-4 items-center border-t border-gray-400 pt-6'>
          <img
            src={listing.images?.[0] || '/placeholder.svg?height=96&width=96&text=Listing+Image'}
            alt='Ảnh đặt chỗ'
            className='w-24 h-24 object-cover rounded-lg border border-gray-200'
          />
          <div className='flex-1'>
            <div className='font-semibold text-lg leading-tight'>{listing.title}</div>
        ...
        <div className='border-t border-gray-400  pt-6 space-y-3'>
          ...
          <div className='flex justify-between text-base text-gray-700'>
            <span>
              ...
              {paymentType === 'deposit' && <span className='ml-2 text-xs text-pink-600'>(Chỉ tính 50%)</span>}
            </span>
            <span>₫{paymentType === 'deposit' ? Math.round(base * 0.5).toLocaleString() : base.toLocaleString()}</span>
          </div>
          {weekendSurcharge > 0 && (
          ...
          <div className='border-t border-gray-400 pt-2'>
            <div className='flex justify-between font-bold text-base'>
              ...
              <span>
                ₫
                {paymentType === 'deposit'
                  ? Math.round((totalRoomPrice + selectedServiceTotal) * 0.5).toLocaleString()
            ...
            <div className='flex justify-between text-base text-gray-700'>
              <span>Thuế (8%)</span>
              <span>₫{paymentType === 'deposit' ? Math.round(tax * 0.5).toLocaleString() : tax.toLocaleString()}</span>
            </div>
            {discount > 0 && (
              <div className='flex justify-between text-base text-green-600 font-medium'>
                ...
                <span>
                  -₫
                  {paymentType === 'deposit' ? Math.round(discount * 0.5).toLocaleString() : discount.toLocaleString()}
                </span>
              </div>
            )}
            <div className='border-t border-gray-400 pt-2 mt-2'>
              ...
              {paymentType === 'deposit' && (
                <div className='text-xs text-gray-500 mt-1'>
                  Bạn sẽ thanh toán trước 50% tổng tiền, phần còn lại sẽ thanh toán sau.
                </div>
              )}
            </div>
          </div>
