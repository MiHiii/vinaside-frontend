...
const VoucherListForUser: React.FC<Props> = ({
  onVoucherSelect,
  totalAmount,
  disabled,
}) => {
  ...
  useEffect(() => {
    ...
    voucherApi
      ...
      .then((res) => {
        console.log(
          "[Voucher FE] API /vouchers/valid response:",
          res.data.data
        );
  ...
  const handleApplyCode = () => {
    ...
    setApplyError(null);
    voucherApi
      .getByCode(inputCode.trim().toUpperCase())
      .then((res) => {
  ...
  if (disabled) {
    return (
      <div className="bg-white rounded-lg shadow p-4 mb-6 opacity-60 pointer-events-none">
        <div className="font-semibold text-lg mb-2">Chọn Voucher</div>
        <div className="text-gray-400 text-sm">
          Không thể áp dụng voucher khi chọn đặt cọc 50%
        </div>
      </div>
    );
  }
  return (
    <div className="bg-white rounded-lg shadow p-4 mb-6">
      <div className="mb-4">
        ...
        {applyError && (
          <div className="text-red-500 text-sm mt-1">{applyError}</div>
        )}
      </div>
      ...
      <div className="space-y-4 max-h-72 overflow-y-auto pr-2">
        {vouchers.length === 0 && (
          <div>Hiện không có voucher nào khả dụng.</div>
        )}
            ...
            >
              <div className="flex flex-col items-center justify-center min-w-[70px]">
                ...
                <div className="text-xs text-gray-600">
                  Tối đa {voucher.max_uses_per_user || 1} lần/người
                </div>
              </div>
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-700 mb-1">
                  Giảm tối đa cho đơn từ ₫
                  {voucher.min_order_value?.toLocaleString() || 0}
                </div>
                {voucher.description && (
              ...
              {notEnough && (
                <div className="text-xs text-red-500 font-semibold">
                  Đơn hàng phải từ ₫{voucher.min_order_value?.toLocaleString()}{" "}
                  mới dùng được
                </div>
              )}
            </div>
