...
export const calendarApi = {
  ...
  getCalendarData: async (params: CalendarQueryParams): Promise<CalendarData> => {
    try {
      ...
      console.log('📅 Calendar API URL:', `${endpoint}?${queryParams.toString()}`);
  ...
  getDayBookings: async (date: string, params?: { propertyId?: string }): Promise<DayDetail> => {
    const queryParams = new URLSearchParams();
    if (params?.propertyId) queryParams.append('propertyId', params.propertyId);

    const response = await api.get(`/bookings/calendar/day/${date}?${queryParams.toString()}`);
    return response.data;
  },

  // Lấy booking theo tuần (cho widget)
  getWeekBookings: async (startDate: string, endDate: string, propertyId?: string): Promise<Booking[]> => {
    const queryParams = new URLSearchParams();
    queryParams.append('startDate', startDate);
    queryParams.append('endDate', endDate);
    if (propertyId) queryParams.append('propertyId', propertyId);

    const response = await api.get(`/bookings/calendar/week?${queryParams.toString()}`);
  ...
  getCalendarStats: async (
    ...
    const response = await api.get(`/bookings/calendar/stats?${queryParams.toString()}`);
    return response.data;
  },
};
