...
class ChatService {
  ...
  async getUsers() {
    try {
      const response = await api.get<AvailableUsersResponse>(
          ...
          (
            ...
            receiverId:
              typeof msg.receiver_id === "object"
                ...
                : msg.receiver_id,
            senderInfo:
              typeof msg.sender_id === "object" ? msg.sender_id : undefined,
            receiverInfo:
              typeof msg.receiver_id === "object" ? msg.receiver_id : undefined,
            ...
            reactions:
              msg.reactions?.map((reaction) => ({
                ...
                userName:
                  typeof reaction.user_id === "object"
                    ? reaction.user_id.name
                    : "",
                createdAt: reaction.created_at,
                type:
                  (reaction.reaction_type as ReactionType) || ReactionType.LIKE,
              })) || [],
          })
        );
      }
  ...
  async markConversationAsRead(otherUserId: string, currentUserId?: string) {
    try {
      ...
      if (unreadMessages.length > 0) {
        ...
        if (failureCount === 0) {
          console.log(
            `✅ All ${successCount} messages marked as read successfully`
          );
          return {
            success: true,
          ...
          console.warn(
            `⚠️ ${successCount} messages marked as read, ${failureCount} failed`
          );
          return {
            success: true,
  ...
  async toggleReaction(messageId: string, reactionType: ReactionType) {
    try {
      const response = await api.post(
        `/messages/${messageId}/reactions/toggle/${reactionType}`
      );
