              ...
              <div className='mt-2 flex gap-2'>
                ...
                {userRole === 'admin' && (
                  <Button
                    size='sm'
                    variant='outline'
                    className='rounded-full h-8 px-3 bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100 transition'>
                    👑 Admin View
                  </Button>
                )}
              </div>
                  ...
                  conversations.map((c: ConversationUI) => (
                    <button
                      ...
                      <Avatar className='h-12 w-12 ring-1 ring-gray-200'>
                        <AvatarImage src={c.display.avatar_url || '/placeholder.svg'} alt={c.display.title} />
                        <AvatarFallback>{c.display.title?.charAt(0) || 'U'}</AvatarFallback>
                      </Avatar>
                      <div className='min-w-0 flex-1'>
                        <div className='flex items-center justify-between'>
                          <h3 className='font-medium truncate text-foreground'>{c.display.title}</h3>
                          {c.lastMessage && (
                        ...
                        {userRole === 'admin' && c.lastMessage && (
                          <div className='mt-1'>
                            <span
                              className={`text-[10px] px-2 py-[1px] rounded ${
                                c.lastMessage.sender_role === 'admin'
                                  ? 'bg-purple-100 text-purple-600 border border-purple-200'
                                  : c.lastMessage.sender_role === 'staff'
                                  ? 'bg-blue-100 text-blue-600 border border-blue-200'
                                  : 'bg-gray-100 text-gray-600 border border-gray-200'
                              }`}>
                              {c.lastMessage.sender_role === 'admin' && '👑 Admin'}
                              {c.lastMessage.sender_role === 'staff' && '👨‍💼 Staff'}
                              {c.lastMessage.sender_role === 'guest' && '👤 Guest'}
                            </span>
                          </div>
                        )}
                        {c.display.badge && (
                ...
                <div className='h-19 px-6 flex items-center justify-between bg-card border-b border-gray-200 flex-shrink-0'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='min-w-0'>
                      <div className='font-medium truncate text-foreground'>{selectedConversation.display?.title}</div>
                            ...
                            {!isMine &&
                              (showSenderMeta ? (
                                <Avatar className='h-8 w-8 mt-5'>
                                  <AvatarImage src={m.ui?.sender_avatar_url || '/placeholder.svg'} />
                            ...
                            <div className={`max-w-[72%] ${isMine ? '' : ''}`}>
                              ...
                              <div className={`mb-1 ${isMine ? 'text-right' : 'text-left'}`}>
                                {!isMine && showSenderMeta && (
                                  <span className='text-[13px] font-medium'>{m.ui?.sender_display_name}</span>
                                )}
                                <span
                                            ...
                                            <>
                                              ...
                                              {userRole === 'admin' && (
                                                <>
                                                  <DropdownMenuItem
                                                    onClick={() => {
                                                      console.log('View user info:', m.sender_id);
                                                    }}
                                                  ...
                                                  <DropdownMenuItem
                                                    onClick={() => {
                                                      console.log('Delete message as admin:', m._id);
                                                    }}
                                                    className='text-destructive focus:text-destructive'>
                                                    <Trash2 className='h-4 w-4 mr-2' />
                                                    Xóa tin nhắn (Admin)
