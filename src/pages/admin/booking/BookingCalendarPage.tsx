...
export const BookingCalendarPage: React.FC = () => {
  ...
  const fetchCalendarData = useCallback(async () => {
    ...
    try {
      ...
      if (isStaff && user?._id) {
        if (filters.propertyId === 'all' && staffProperties.length > 0) {
          const staffPropertyIds = staffProperties
            .map((item: { propertyId?: { _id: string } }) => item.propertyId?._id)
            .filter(Boolean);
      ...
      const params: Partial<CalendarQueryParams> & { keyword?: string } = {
        ...
        ...((filters as any).searchType === 'guestName' ? { guestName: filters.keyword || undefined } : {}),
        ...((filters as any).searchType === 'listingTitle' ? { listingTitle: filters.keyword || undefined } : {}),
        ...((filters as any).searchType === 'propertyName' ? { propertyName: filters.keyword || undefined } : {}),
        ...((filters as any).searchType === 'keyword' || !(filters as any).searchType
  ...
  useEffect(() => {
    ...
    fetchCalendarData();
  }, [filters, dispatch, propertiesFromStore.length, fetchCalendarData, isStaff]);
  ...
  const handleBookingClick = (bookingId: string, propertyId: string) => {
    ...
    if ((!propertyId || propertyId === 'unknown' || propertyId === 'all') && isStaff && staffProperties.length > 0) {
      const firstProperty = staffProperties[0];
      finalPropertyId = firstProperty.propertyId?._id || firstProperty.propertyId?.id;
    }

    // Nếu vẫn không có propertyId hợp lệ và không phải staff, sử dụng propertyId đầu tiên từ properties
    if (
      (!finalPropertyId || finalPropertyId === 'unknown' || finalPropertyId === 'all') &&
      !isStaff &&
      properties.length > 0
    ) {
    ...
    if (!finalPropertyId || finalPropertyId === 'unknown' || finalPropertyId === 'all') {
  ...
  return (
    <div className='container mx-auto px-6 py-8 bg-gray-50 min-h-screen'>
      {/* Page Header */}
      <BookingHeader title='Quản lý Booking' subtitle='Xem và quản lý tất cả booking theo lịch' />
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold text-gray-900'>{loading ? '...' : calendarData?.totalBookings || 0}</p>
              </div>
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold text-gray-900'>
                  {loading ? '...' : Math.round(calendarData?.totalRevenue || 0).toLocaleString('vi-VN') + 'đ'}
                </p>
              </div>
              <div className='h-10 w-10 bg-gray-100 rounded-lg flex items-center justify-center'>
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold text-gray-900'>
                  {loading ? '...' : (calendarData?.averageOccupancy || 0).toFixed(2) + '%'}
                </p>
              </div>
              <div className='h-10 w-10 bg-gray-100 rounded-lg flex items-center justify-center'>
      ...
      {/* Filters */}
      <CalendarFilter filters={filters} onFiltersChange={handleFiltersChange} properties={properties} />
