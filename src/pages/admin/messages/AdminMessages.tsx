...
export default function AdminMessages() {
  ...
  // Load messages for a conversation without changing selection
  ...
  useEffect(() => {
    if (selectedConversation && messages.length > 0 && !isLoadingMessages) {
      setTimeout(() => {
        if (messagesEndRef.current) {
          messagesEndRef.current.scrollIntoView({
            behavior: 'smooth',
  ...
  }, [selectedConversation?._id, messages.length, isLoadingMessages]);
