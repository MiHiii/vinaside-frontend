...
export default function StaffMessages() {
  ...
  const loadConversationsInternal = useCallback(async () => {
    if (isLoadingConversations) {
      console.log('🔄 [StaffMessages] Already loading conversations, skipping...');
      return;
    }
  ...
  const selectConversation = useCallback(
    async (conversation: ConversationUI) => {
      try {
        console.log('🎯 [StaffMessages] Selecting conversation:', conversation._id);
        ...
        console.log('📨 [StaffMessages] Loading messages for conversation:', conversation._id);
        ...
        if (cachedMessages.length > 0) {
          console.log('📨 [StaffMessages] Using cached messages:', cachedMessages.length);
          ...
          console.log(
            '📨 [StaffMessages] Processed cached messages with ownership:',
          ...
          try {
            ...
            if (readResponse.ok) {
              ...
              console.log('✅ [StaffMessages] Conversation marked as read:', conversation._id);
            }
          } catch (error) {
            console.error('❌ [StaffMessages] Error marking conversation as read:', error);
          }
          return;
        }
        ...
        console.log(
          '📥 [StaffMessages] Processed messages with ownership:',
        ...
        console.error('❌ [StaffMessages] Error selecting conversation:', error);
      } finally {
        setIsLoadingMessages(false);
      }
    },
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      console.log('📤 [StaffMessages] Created messageWithUI:', {
  ...
  useEffect(() => {
    ...
    console.log('🟢 [StaffMessages] Setting up socket connection for:', userRole);
    ...
    const setupRealtimeListeners = () => {
      const handleNewMessage = (message: MessageWithUI) => {
        ...
        console.log('📨 [StaffMessages] Sender display name:', message.ui?.sender_display_name);
        console.log('📨 [StaffMessages] Message conversation_id:', message.conversation_id);
        console.log('📨 [StaffMessages] Selected conversation ID:', selectedConversation?._id);
              ...
              return prev.map((m) => {
                if (m._id === message._id) {
                  ...
                  if (
                    ...
                    console.log('🔒 [StaffMessages] Preserving ui.mine for existing message:', message._id);
                  }
      ...
      const handleConversationUpdate = (data: ConversationUpdateV2) => {
        ...
        console.log('💬 [StaffMessages] Update type:', data.isAdminUpdate ? 'admin' : 'regular');
        ...
        if (isLoadingConversations) {
          console.log('⏳ [StaffMessages] Skipping conversation update - currently loading');
          return;
        }
        ...
        if (data.lastMessageAt && selectedConversationRef.current?._id === data.conversationId) {
          console.log('� [StaffMessages] Received update for current conversation, waiting for new_message event');
        }
      ...
      const handleConversationUpdateV2 = (conversationUpdate: ConversationUpdateV2) => {
        ...
        console.log('🔄 [StaffMessages] Received conversation_update_v2:', conversationUpdate);
        setConversations((prev) =>
          prev.map((conv) =>
      ...
      const handleReactionUpdate = (message: MessageWithUI) => {
        console.log('😀 [StaffMessages] Received reaction update:', message._id);
        if (selectedConversationRef.current?._id === message.conversation_id) {
          setMessages((prev) =>
            prev.map((msg) =>
              msg._id === message._id
      ...
      const handleConversationListUpdate = (data: ConversationUpdateV2) => {
        console.log('📋 [StaffMessages] Received conversation list update:', data);

        // Rate limiting for conversation list updates
        const now = Date.now();
        if (now - lastUpdateTimeRef.current < 1000) {
          console.log('⏳ [StaffMessages] Rate limiting - skipping conversation list update');
          return;
        }
        lastUpdateTimeRef.current = now;
    ...
    if (isAdminOrStaff) {
      console.log('👑 [StaffMessages] Joining admin_broadcast room for:', userRole);
      setTimeout(() => {
        ...
        console.log('🔍 [StaffMessages] Socket status:', socketService.getConnectionStatus());
      }, 1000);
    }
  ...
  useEffect(() => {
    if (token && user) {
      console.log('🚀 [StaffMessages] Initializing conversations for staff:', user.name);
  ...
  return (
    <div className='flex min-h-min bg-background justify-center mt-6 mb-6' style={{ scrollBehavior: 'auto' }}>
      <div className='flex w-full max-w-[1450px] h-full overflow-hidden rounded-lg shadow-lg'>
        ...
        <div className='w-100 bg-card border-r border-gray-200 flex flex-col'>
          <div className='px-4 py-4 border-b border-gray-200 flex-shrink-0 flex justify-between items-center'>
            <h1 className='text-[17px] font-semibold text-foreground'>Tin nhắn của nhân viên</h1>
            ...
            <div className='p-2'>
              {isLoadingConversations ? (
                ...
                <div className='text-center py-8'>
                  <p className='text-muted-foreground'>Chưa có cuộc hội thoại nào được gán</p>
                </div>
              ) : (
              ...
              <ScrollArea
                ...
                <div className='px-6 py-4 max-w-4xl mx-auto' style={{ scrollBehavior: 'auto' }}>
                  {isLoading ? (
                    ...
                    <div className='text-center py-8'>
                      <p className='text-muted-foreground'>Chưa có tin nhắn nào (messages.length: {messages.length})</p>
                    </div>
                  ) : (
                    (() => {
                      ...
                      console.log('🎨 [StaffMessages] Rendering messages, count:', messages.length);
                      console.log('🎨 [StaffMessages] Messages state:', messages);
                      ...
                      uniqueMessages.forEach((m: MessageWithUI, idx: number) => {
                        ...
                        console.log('🔍 [StaffMessages] Message ownership check:', {
                          messageId: m._id,
