...
export default function UserProfilePage() {
  ...
  const handleSidebarSelect = (item: string) => {
    ...
    // Trong tương lai, bạn có thể thêm logic fetch dữ liệu tương ứng với item ở đây
  };
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      <div className="flex flex-col lg:flex-row lg:space-x-12">
        ...
        <main className="flex-1 lg:w-3/4 xl:w-4/5 lg:ml-[200px]">
          {activeSidebarItem === "introduction" && (
            <section aria-labelledby="introduction-section-title">
             <div className="flex items-center gap-2 mb-6">
              <h1 id="introduction-section-title" className="text-2xl sm:text-3xl font-bold">
                Giới thiệu bản thân
              </h1>
            <Button
            size="sm"
          ...
          >
            <Link to="/edit-profile" className="flex items-center justify-center w-full h-full">
              Chỉnh sửa
            </Link>
          </Button>
              ...
              {/* Trên màn hình lớn, chúng có thể nằm cạnh nhau */}
              ...
              {/* Phần "Danh giá đã giới thiệu" hoặc thông tin xác minh */}
              <div className="mt-10 pt-8 border-t border-gray-200">
          
                 <button
                className="w-[200px] h-[50px] flex items-center space-x-2 hover:bg-gray-100 p-2 rounded-xl focus:outline-none"
                onClick={() => {
                  // Xử lý hành động khi click vào nút tại đây
                  console.log("Đánh giá tôi đã viết clicked");
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true" role="presentation" focusable="false" className="block h-[24px] w-[24px] fill-current">
            ...
            <section aria-labelledby="trips-section-title">
              <h1 id="trips-section-title" className="text-2xl sm:text-3xl font-bold mb-6">
                Chuyến đi trước đây
              </h1>
              <PastTrip /> {/* Gọi component PastTrip bên trong */}
            </section>
          )}
          
             {activeSidebarItem === "reviews" && (
            <section aria-labelledby="connection-section-title">
              <h1 id="connection-section-title" className="text-2xl sm:text-3xl font-bold mb-6">
              Kết nối
              </h1>
              <Connection /> {/* Gọi component PastTrip bên trong */}
            </section>
          )}
